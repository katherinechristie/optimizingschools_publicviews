...
# Inspect the unmerged observations with no match in the prolific data
# Results in 22 observations that don't merge 
table(raw_data_wp$is_failmerge_prol, useNA = "always") 

raw_data_wp_unmatched = raw_data_wp %>%
  filter(is_failmerge_prol)

table(raw_data_wp_unmatched$is_prolid_wronglength, useNA = "always")
...
# Review flags 
table(raw_data_wp_intermed$political_affil, useNA = "always")
table(raw_data_wp_cleanvar$political_affil_buckets, useNA = "always")

temp <- raw_data_wp_cleanvar %>%
  select(contains("jw"))
```
...
```{r}
## save
stargazer(lm(points_rank_rev ~ is_any_barriers,
           data = model_df),
...
```{r}
## save
stargazer(lm(points_rank_rev ~ cond,
           data = barriers_r),
...
## minority
summary(lm(points_rank_rev ~ relevel(factor(cond),
                                    ref = "Wom")*is_minority,
           data = barriers_r))
...
predict_jw_df = data.frame(is_any_barriers = rep(c(TRUE, FALSE),
                                                 each = 100),
                           jw_combined = rep(seq(from = min(model_df$jw_combined),
                                                 to = max(model_df$jw_combined),
                                                 length.out = 100), 
                                             2)) 
...
```{r}
summary(lm(points_rank_rev ~ is_any_barriers*relevel(factor(political_affil_buckets),
                                                     ref = "Democrat"),
           data = model_df))
...
```{r}
summary(lm(points_rank_rev ~ is_any_barriers*political_ideology_numeric,
           data = model_df))
