...
code_agg_scales <- function(data, prefix_string,
  ...
  ### rj note-- right now, if respondent leaves an item blank (NA) it's
...
# Load qualtrics data
raw_data_init = read_survey("./data/Beliefs about funding models _finalv_January 6, 2021_17.21.csv")
...
# Load prolific dem
prol_dem = read.csv("./data/prolific_export_5fd1326bfc1d530abecbcd16.csv")
...
```{r}
fr_coded_z = read.csv("./data/frcoding_finalsurvey - Xinzhe.csv")
fr_coded_k = read.csv("./data/frcoding_finalsurvey - K to code.csv") %>%
...
nonsense_codes = read.csv("./data/frcoding_finalsurvey - flag_nonsensical.csv")
...
dist_rank_plot
ggsave("../output/jap_paper/figs/rankings_count_3groups.png",
       plot = dist_rank_plot,
       width = 12,
       height = 8) 
...
prop_choose_plot
ggsave("../output/jap_paper/figs/rankings_prop_2groups.png",
...
ggsave("../output/jap_paper/figs/rankings_prop_3groups.png",
...
ggsave("../output/jap_paper/figs/rankings_prop_3groups_wpoliticalaffil.png",
...
ggsave("../output/jap_paper/figs/rankings_prop_3groups_wpoliticalaffil_excludeother.png",
...
ggsave("../output/jap_paper/figs/among_points_timedist.png",
       plot = time_plot,
       width = 12,
       height = 8) 
...
summary(lm(points_rank ~ is_any_barriers,
           data = model_df))
...
```{r}
## save
stargazer(lm(points_rank ~ is_any_barriers,
           data = model_df),
          lm(is_points_first ~ is_any_barriers,
           data = model_df),
          type = "html",
          out="../output/jap_paper/tables/maineffect_regs.doc")
...
```{r}
## save
stargazer(lm(points_rank ~ cond,
           data = barriers_r),
          lm(is_points_first ~ cond,
           data = barriers_r),
          type = "html",
          out="../output/jap_paper/tables/womenvmob_barriers_regs.doc")
...
ggsave("../output/jap_paper/figs/jw_prediction.png",
...
ggsave("../output/jap_paper/figs/pol_prediction.png",
...
write.table(prop_themes_all, 
    file = "../output/jap_paper/tables/overall_fr_themes.txt",
    sep = ",", quote = FALSE, row.names = F)
write.csv(prop_themes_all, 
    file = "../output/jap_paper/tables/overall_fr_themes.csv",
...
write.table(prop_by_cond, 
    file = "../output/jap_paper/tables/themes_by_treatcondition.txt",
    sep = ",", quote = FALSE, row.names = F)
write.csv(prop_by_cond, 
    file = "../output/jap_paper/tables/themes_by_treatcondition.csv",
     quote = FALSE, row.names = F)
...
write.table(prop_by_rank, 
    file = "../output/jap_paper/tables/themes_by_pointsranking.txt",
    sep = ",", quote = FALSE, row.names = F)
write.csv(prop_by_rank, 
    file = "../output/jap_paper/tables/themes_by_pointsranking.csv",
     quote = FALSE, row.names = F)
...
ggsave("../output/jap_paper/figs/prop_frthemes_byrank.png",
       plot = last_plot(),
       width = 12,
       height = 8) 
...
ggsave("../output/jap_paper/figs/prop_frthemes_byrank_byideology.png",
       plot = last_plot(),
       width = 12,
       height = 8) 
```
...
```{r}
## add later; since some are not continuous etc

```


### Addtional code for waiving the Supplement
```{r}
## table to summarize dummies in filtering from raw_data_wp_cleanvar
dummies_explained <- data.frame(dummy_var = c("is_nonsens_answer",
                                     "is_cons_revok",
                                     "is_missing_DV",
                                     "is_failattention",
                                     "is_wrong_distype",
                                     ...
                                     "is_non100_progress",
                                      "is_exact_dup_fr"), 
                                sample_perc = c('2.17%', '2.00%', '8.02%','4.17%','0.08%',
                                                '1.76%', '0.72%', '2.97%', '0.96%'),
                                explanation = c("","","","","","","","",""))
xtable(dummies_explained)
```
...
stargazer(lm(points_rank ~ is_any_barriers,
          ...
          #,out="../output/jap_paper/latexTables/maineffect_regs.doc")
...
stargazer(lm(points_rank ~ cond,
          ...
          lm(is_points_first ~ cond,
           data = barriers_r),
          type = "latex"
          #, out="../output/jap_paper/latexTables/womenvmob_barriers_regs.doc"
         )

## hypothesis 4: 
stargazer(lm(points_rank ~ cond * is_woman,
             data = barriers_r),
          lm(points_rank ~ relevel(factor(cond),
                                    ref = "Wom")*is_minority,
           data = barriers_r),
          lm(points_rank ~ relevel(factor(cond),
                                    ref = "Wom")*is_minority_sec,
           data = barriers_r),
          type = "latex"
          #, out="../output/jap_paper/latexTables/women_min_barriers_regs.doc"
         )

## hypothesis 5
## 5.1 Just world
stargazer(lm(points_rank ~ is_any_barriers*jw_combined,
           data = model_df),
          lm(is_points_first ~ is_any_barriers*jw_combined,
           data = model_df),
          type = "latex"
          #, out="../output/jap_paper/latexTables/js_world_barriers_regs.doc"
         )

##5.2 political affiliation
stargazer(lm(points_rank ~ is_any_barriers*relevel(factor(political_affil_buckets),
                                                     ref = "Democrat"),
           data = model_df),
          lm(is_points_first ~ is_any_barriers*relevel(factor(political_affil_buckets),
                                                     ref = "Democrat"),
           data = model_df),
          type = "latex"
          #, out="../output/jap_paper/latexTables/poli_aff_barriers_regs.doc"
         )

##5.3 political ideo
stargazer(lm(points_rank ~ is_any_barriers*political_ideology_numeric,
           data = model_df),
          lm(is_points_first ~ is_any_barriers*political_ideology_numeric,
           data = model_df),
          type = "latex"
          #, out="../output/jap_paper/latexTables/poli_aff_barriers_regs.doc"
         )
...
## demographic plot on jw scale, political ideology and affiliation
```{r}

## a box plot for jw_combined vs. political ideology
...
ggsave("../output/jap_paper/figs/jw_poli_ideo_plot.png",
       plot = jw_poli_ideo_plot,
       width = 12,
       height = 8) 

#histogram of bjw scores faceted by ideology
jw_poli_affil_plot <- ggplot(model_df, aes(x=jw_combined, na.rm = TRUE)) +
...
ggsave("../output/jap_paper/figs/jw_poli_affil_plot.png",
